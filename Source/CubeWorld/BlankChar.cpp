...
ABlankChar::ABlankChar()
{
	...
	// We drop it down by the half-height (-88.0f) so the feet touch the floor.
	// We also rotate it -90 degrees so the character faces "forward" appropriately.
	...
	CameraBoom->bUsePawnControlRotation = true; // Rotate the arm based on the controller input

	// Create a follow camera attached to the end of the boom
	FollowCamera = CreateDefaultSubobject<UCameraComponent>(TEXT("FollowCamera"));
	FollowCamera->SetupAttachment(CameraBoom, USpringArmComponent::SocketName);
	FollowCamera->bUsePawnControlRotation = false; // Camera itself does not rotate, the boom does


	// Attach Header statically to the 'head' bone
	HeadMesh = CreateDefaultSubobject<UStaticMeshComponent>(TEXT("HeadMesh"));
	HeadMesh->SetupAttachment(GetMesh(), FName("head"));
	static ConstructorHelpers::FObjectFinder<UStaticMesh> HeadAsset(TEXT("/Script/Engine.StaticMesh'/Game/BlankChar/Head.Head'"));
	if (HeadAsset.Succeeded()) HeadMesh->SetStaticMesh(HeadAsset.Object);
	...
	static ConstructorHelpers::FObjectFinder<UStaticMesh> TorsoAsset(TEXT("/Script/Engine.StaticMesh'/Game/BlankChar/Torso.Torso'"));
	if (TorsoAsset.Succeeded()) TorsoMesh->SetStaticMesh(TorsoAsset.Object);

	// Attach hands
	HandLMesh = CreateDefaultSubobject<UStaticMeshComponent>(TEXT("HandLMesh"));
	HandLMesh->SetupAttachment(GetMesh(), FName("hand_l"));
	static ConstructorHelpers::FObjectFinder<UStaticMesh> HandLAsset(TEXT("/Script/Engine.StaticMesh'/Game/BlankChar/Hand_L.Hand_L'"));
	if (HandLAsset.Succeeded()) HandLMesh->SetStaticMesh(HandLAsset.Object);

	HandRMesh = CreateDefaultSubobject<UStaticMeshComponent>(TEXT("HandRMesh"));
	HandRMesh->SetupAttachment(GetMesh(), FName("hand_r"));
	static ConstructorHelpers::FObjectFinder<UStaticMesh> HandRAsset(TEXT("/Script/Engine.StaticMesh'/Game/BlankChar/Hand_R.Hand_R'"));
	if (HandRAsset.Succeeded()) HandRMesh->SetStaticMesh(HandRAsset.Object);

	// Attach feet (legs)
	FootLMesh = CreateDefaultSubobject<UStaticMeshComponent>(TEXT("FootLMesh"));
	FootLMesh->SetupAttachment(GetMesh(), FName("foot_l"));
	static ConstructorHelpers::FObjectFinder<UStaticMesh> FootLAsset(TEXT("/Script/Engine.StaticMesh'/Game/BlankChar/Foot_L.Foot_L'"));
	if (FootLAsset.Succeeded()) FootLMesh->SetStaticMesh(FootLAsset.Object);

	FootRMesh = CreateDefaultSubobject<UStaticMeshComponent>(TEXT("FootRMesh"));
	FootRMesh->SetupAttachment(GetMesh(), FName("foot_r"));
	static ConstructorHelpers::FObjectFinder<UStaticMesh> FootRAsset(TEXT("/Script/Engine.StaticMesh'/Game/BlankChar/Foot_R.Foot_R'"));
	if (FootRAsset.Succeeded()) FootRMesh->SetStaticMesh(FootRAsset.Object);
...
void ABlankChar::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
...
void ABlankChar::Turn(float Value)
{
	AddControllerYawInput(Value);
}

void ABlankChar::LookUp(float Value)
{
